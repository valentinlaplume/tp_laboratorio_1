...
static int flagYaAbriArchivo = 0;
...
static int preguntarAbrirArchivo(int flagAbriElArchivo);
...
int controller_loadFromText(char* path, LinkedList* pArrayListEmployee)
{
	...
	if(path != NULL && pArrayListEmployee != NULL)
	{
		...
		if(pFile != NULL)
		{
			respuestaAbrirArchivo = preguntarAbrirArchivo(flagYaAbriArchivo);
			if(!respuestaAbrirArchivo || !flagYaAbriArchivo)
			{
...
int controller_ListEmployee(LinkedList* pArrayListEmployee)
{
	...
	if(pArrayListEmployee != NULL && lenLinkedList > 0)
	{
		...
		printf("\n\x1b[31m *\x1b[0m Error, no hay empleados cargados por listar");
	return retorno;
}
...
int controller_sortEmployee(LinkedList* pArrayListEmployee)
{
	...
	if(!ll_sort(pArrayListEmployee, employee_compararPorNombre, 1))
	{
		if(!controller_ListEmployee(pArrayListEmployee))
		{
			printf("\n\x1b[92m >\x1b[0m Ordenado por nombre [A - Z]");
			retorno = 0;
		}
	}
	else
 ...
 */
static int preguntarAbrirArchivo(int flagAbriArchivo)
{
	int retorno = -1;
	int respuesta;
	if(flagAbriArchivo == 1)
	{
		respuesta = laplume_confirmarAccion("\n\x1b[33m **************************** ATENCION **************************** \x1b[0m"
                							"\n > El Archivo ya fue cargado, si lo carga denuevo puede que haya IDs que se repitan y que causen problemas futuros"
											"\n > De todas formas, desea cargar archivo?");
		retorno = respuesta;
	}
	return retorno;
}
